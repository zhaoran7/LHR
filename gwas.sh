...
threads=40
impdir=/work/data/ukb/gen/imp
label=height; binary=N
pheno_f=/work/data/ukb/phe/ukb.pheno; phenos="height leg lhr.res"
covar_f=$pheno_f; covars="age,sex,PC1-PC4"
...
# step1: run PLINK 🏃‍
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
outdir=/work/data/gwas/self/$label; mkdir -p $outdir
for chr in {1..22} X; do
	x_str=""; if [[ $chr == X ]]; then x_str="--xchr-model 2"; fi
	
	echo "#!/bin/bash
	source /work/scripts/f/phe.f.sh
